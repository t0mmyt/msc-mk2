...
class SAX(object):
    ...
    def __init__(self, v, alphabet, absolute=False, distribution="gaussian"):
        self.v = v
        self.alphabet = alphabet
        n = len(alphabet)
        if distribution == "gaussian":
            if absolute:
                a = np.array(SAX.gaussian_dist[len(alphabet) * 2])
                self.breakpoints = a[a>0]
            else:
                self.breakpoints = np.array(SAX.gaussian_dist[len(alphabet)])
        elif distribution == "proportional":
            if absolute:
                self.breakpoints = [round((i + 1) * (1 / n), 3) for i in range(n - 1)]
            else:
                self.breakpoints = [round(2 * (i + 1) * (1 / n) - 1, 3) for i in range(n - 1)]
        else:
            raise SaxError("Distribution type {} was unknown".format(distribution))
        self.breakpoints = list(self.breakpoints)

    def __iter__(self):
        for y in self.v:
            yield self.alphabet[np.searchsorted(self.breakpoints, y)]
